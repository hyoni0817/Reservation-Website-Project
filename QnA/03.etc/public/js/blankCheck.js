function submitCheck(stat) {
  var userName = document.getElementById("userName").value;
  var phoneNum = $('#phoneNum').val();
  var email = $('#emailText').val();
  var regex = /^(\s|\d)+$/;
  var emailRegex = /^[A-Za-z0-9_\.\-]+@[A-Za-z0-9\-]+\.[A-Za-z0-9\-]+/; //이메일 형식을 입력했는지 체크
  ...
  if($('#userName').val().replace(/\s/g,"").length == 0) {
  ...
  } else if($('#phoneNum').val().replace(/\s/g,"").length == 0) {
    ...
    alertEle[0].innerHTML = alertHTML;
  } else if($('#emailText').val().replace(/\s/g,"").length == 0) {
    ...
    alertEle[0].innerHTML = alertHTML;
  } else if(stat == 'origin' && termsCheck.checked == false) {
    ...
    let frm = document.getElementById("edit-person-info");
    frm.action="/editReser/step1";
    frm.method="post";
    frm.submit();
  }
